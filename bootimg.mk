...
recoveryasboot_ramdisk := $(PRODUCT_OUT)/ramdisk-recoveryasboot.img
INSTALLED_RECOVERYASBOOTIMAGE_TARGET := $(PRODUCT_OUT)/recoveryasboot.img
...
$(recoveryasboot_ramdisk).mtk.header: $(recovery_ramdisk)
	size=$$($(call get-file-size,$(recovery_ramdisk))); \
		$(call make_header, $$((size)), "ROOTFS", $@)
$(recoveryasboot_ramdisk).mtk:  $(MKBOOTIMG) $(recoveryasboot_ramdisk).mtk.header
	$(call pretty,"Adding MTK header to recovery ramdisk.")
	cat $(recoveryasboot_ramdisk).mtk.header $(recovery_ramdisk) > $@
...
ifdef BOARD_MTK_KERNEL_CMDLINE
	INTERNAL_MTK_BOOTIMAGE_ARGS += --cmdline "$(BOARD_MTK_KERNEL_CMDLINE)"
endif
...
ifdef BOARD_MTK_KERNEL_CMDLINE
	INTERNAL_MTK_RECOVERYIMAGE_ARGS += --cmdline \
		"$(BOARD_MTK_KERNEL_CMDLINE) androidboot.selinux=permissive"
endif
...
INTERNAL_MTK_RECOVERYASBOOTIMAGE_ARGS := \
		--kernel $(INSTALLED_KERNEL_TARGET).mtk \
		--ramdisk $(recoveryasboot_ramdisk).mtk

ifdef BOARD_MTK_KERNEL_CMDLINE
	INTERNAL_MTK_RECOVERYASBOOTIMAGE_ARGS += --cmdline \
		"$(BOARD_MTK_KERNEL_CMDLINE) androidboot.selinux=permissive"
endif

$(INSTALLED_RECOVERYASBOOTIMAGE_TARGET): $(MKBOOTIMG) \
		$(recoveryasboot_ramdisk).mtk $(INSTALLED_KERNEL_TARGET).mtk
	@echo -e ${CL_CYN}"----- Making recovery image ------"${CL_RST}
	$(MKBOOTIMG) $(INTERNAL_MTK_RECOVERYASBOOTIMAGE_ARGS) \
		$(INTERNAL_MKBOOTIMG_VERSION_ARGS) $(BOARD_MKBOOTIMG_ARGS) --output $@
	$(if $(filter true,$(PRODUCTS.$(INTERNAL_PRODUCT).PRODUCT_SUPPORTS_BOOT_SIGNER)),\
		$(BOOT_SIGNER) /boot $@ $(PRODUCTS.$(INTERNAL_PRODUCT).PRODUCT_VERITY_SIGNING_KEY).pk8 \
			$(PRODUCTS.$(INTERNAL_PRODUCT).PRODUCT_VERITY_SIGNING_KEY).x509.pem $@)
